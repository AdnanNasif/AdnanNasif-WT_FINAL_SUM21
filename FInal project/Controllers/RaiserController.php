<?php
	...
	else if (isset($_POST["edit_raiser"]))
	{
		if(empty($_POST["name"]))
		{
			$hasError  = true;
		...
		$rs =  updateRaiser($_POST["name"],$_POST["pass"],$_POST["nid"],$_POST["num"],$_POST["id"]);
		if($rs === true){
			header("Location: allraiser.php");
		}
		$err_db = $rs;
	...
	function insertUser($name,$pass,$nid,$num){
		$query = "insert into raiser values (NULL,'$name','$pass','$nid','$num')";
		return execute($query);
		
	}
	function authenticateUser($uname,$pass){
		$query = "select * from raiser where username='$uname' and password='$pass'";
		$rs = get($query);
		if(count($rs) > 0){
			return true;
		}
	...
	function getAllRaiser(){
		$query = "select * from raiser order by name asc";
		$rs = get($query);
		return $rs;
	}
	function getRaiser($id){
		$query="select * from raiser where id = $id";
		$rs = get($query);
		return $rs[0];
	}
	function updateRaiser($name,$pass,$nid,$num,$id)
	{
		$query="update raiser set name='$name',password='$pass',nid='$nid',num='$num' where id= '$id'";
		return execute($query);
